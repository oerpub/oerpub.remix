# This is a plugin for velruse. It is almost a copy of the google_oauth2
# provider, but it allows extra info to be passed on the state parameter.
...
import requests
from velruse.api import AuthenticationComplete, AuthenticationDenied, \
    register_provider
...
def add_google_login(config, consumer_key=None, consumer_secret=None,
                     scope=None, login_path='/login/google',
...
class GoogleOAuth2Provider(object):
    ...
    def login(self, request, **kw):
        ...
        extrastate = urllib.quote(urllib.urlencode(kw))
    ...
    def callback(self, request):
        ...
        # CSRF variable is in the first 32 characters
        req_state = request.GET.get('state')[:32]
